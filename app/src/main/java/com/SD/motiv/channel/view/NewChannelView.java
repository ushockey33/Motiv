package com.SD.motiv.channel.view;
...
//TODO here is emoji check, change for only valid characters
//        @Override
...
//        }
        @Override
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            System.out.println("CHAR "+s.toString() + " start "+ start+" before "+before + " count "+count);
            isValidInput = sequenceIsValidCharacters(s, 0, count) && inputLengthCheck(count) ;
        }

        private boolean inputLengthCheck(int count){
            if(count > 16){
                return false;
            }
            return true;
...
//        }
        ...
        private boolean sequenceIsValidCharacters(CharSequence sequence, int start, int count) {
        ...
        public void afterTextChanged(Editable s) {
            ...
            System.out.println("Channel Name Length "+s.length());
            System.out.println();
            if (!isValidInput) {
